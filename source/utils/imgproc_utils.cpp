// This is mostly identical to some code in PPSSPP
// Don't be alarmed by that, I (Peter Thoman) originally wrote the vast majority of it
...
namespace {
	...
	void scaleBicubicBSpline(int factor, u32* data, u32* out, int w, int h, int l, int u) {
		...
		if(cpu_info.bSSE4_1) {
			...
			default: SDLOG(-1, "Bicubic upsampling only implemented for factors 2 to 5");
			}
		}
		else {
			#endif
	...
	void bilinearH(int factor, u32* data, u32* out, int w, int l, int u) {
		...
		default: SDLOG(-1, "Bilinear upsampling only implemented for factors 2 to 5");
		}
	}
