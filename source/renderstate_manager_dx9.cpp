...
IDirect3D9* RSManagerDX9::getD3D() {
	IDirect3D9* d3d = NULL;
...
void RSManagerDX9::reloadShaders() {
	if(downsampling) {
		scaler->reloadShader();
	}
...
HRESULT RSManagerDX9::redirectPresentEx(CONST RECT* pSourceRect, CONST RECT* pDestRect, HWND hDestWindowOverride, CONST RGNDATA* pDirtyRegion, DWORD dwFlags) {
...
void RSManagerDX9::dumpSurface(const char* name, IDirect3DSurface9* surface) {
...
void RSManagerDX9::dumpTexture(const char* name, IDirect3DTexture9* tex) {
...
void RSManagerDX9::registerKnowTexture(LPCVOID pSrcData, UINT SrcDataSize, LPDIRECT3DTEXTURE9 pTexture) {
...
void RSManagerDX9::registerD3DXCompileShader(LPCSTR pSrcData, UINT srcDataLen, const D3DXMACRO* pDefines, LPD3DXINCLUDE pInclude, LPCSTR pFunctionName, LPCSTR pProfile, DWORD Flags, LPD3DXBUFFER * ppShader, LPD3DXBUFFER * ppErrorMsgs, LPD3DXCONSTANTTABLE * ppConstantTable) {
...
void RSManagerDX9::restoreRenderState() {
...
const char* RSManagerDX9::getTextureName(IDirect3DBaseTexture9* pTexture) {
#define TEXTURE(_name, _hash) \
...
HRESULT RSManagerDX9::redirectD3DXCreateTextureFromFileInMemoryEx(LPDIRECT3DDEVICE9 pDevice, LPCVOID pSrcData, UINT SrcDataSize, UINT Width, UINT Height, UINT MipLevels, DWORD Usage,
...
HRESULT RSManagerDX9::redirectGetBackBuffer(UINT iSwapChain, UINT iBackBuffer, D3DBACKBUFFER_TYPE Type, IDirect3DSurface9** ppBackBuffer) {
	if(downsampling) {
...
HRESULT RSManagerDX9::redirectGetRenderTarget(DWORD renderTargetIndex, IDirect3DSurface9** ppRenderTarget) {
	HRESULT res = d3ddev->GetRenderTarget(renderTargetIndex, ppRenderTarget);
	SDLOG(2, " -> %p\n", *ppRenderTarget);
	return res;
}

HRESULT RSManagerDX9::redirectGetDisplayMode(UINT iSwapChain, D3DDISPLAYMODE* pMode) {
	HRESULT res;
...
HRESULT RSManagerDX9::redirectGetDisplayModeEx(UINT iSwapChain, D3DDISPLAYMODEEX* pMode, D3DDISPLAYROTATION* pRotation) {
	HRESULT res;
...
namespace {
	...
	void initResourcesWrapper(bool downsampling, const D3DPRESENT_PARAMETERS *pPresentationParameters) {
		RSManager::getDX9().initResources(downsampling,
...
HRESULT RSManagerDX9::redirectCreateDevice(IDirect3D9* d3d9, UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags, D3DPRESENT_PARAMETERS *pPresentationParameters, IDirect3DDevice9 **ppReturnedDeviceInterface) {
...
HRESULT RSManagerDX9::redirectCreateDeviceEx(IDirect3D9Ex* d3d9ex, UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags, D3DPRESENT_PARAMETERS* pPresentationParameters,
...
HRESULT RSManagerDX9::redirectReset(D3DPRESENT_PARAMETERS * pPresentationParameters) {
...
HRESULT RSManagerDX9::redirectResetEx(D3DPRESENT_PARAMETERS* pPresentationParameters, D3DDISPLAYMODEEX* pFullscreenDisplayMode) {
...
void RSManagerDX9::redirectSetCursorPosition(int X, int Y, DWORD Flags) {
...
HRESULT RSManagerDX9::redirectSetVertexShader(IDirect3DVertexShader9* pvShader) {
	return plugin->redirectSetVertexShader(pvShader);
}

HRESULT RSManagerDX9::redirectSetRenderState(D3DRENDERSTATETYPE State, DWORD Value) {
	return plugin->redirectSetRenderState(State, Value);
}
HRESULT RSManagerDX9::redirectSetSamplerState(DWORD Sampler, D3DSAMPLERSTATETYPE Type, DWORD Value) {
	return d3ddev->SetSamplerState(Sampler, Type, Value);
}

HRESULT RSManagerDX9::redirectClear(DWORD Count, CONST D3DRECT *pRects, DWORD Flags, D3DCOLOR Color, float Z, DWORD Stencil) {
	return plugin->redirectClear(Count, pRects, Flags, Color, Z, Stencil);
}
HRESULT RSManagerDX9::redirectDrawPrimitive(D3DPRIMITIVETYPE PrimitiveType, UINT StartVertex, UINT PrimitiveCount) {
	return plugin->redirectDrawPrimitive(PrimitiveType, StartVertex, PrimitiveCount);
}
HRESULT RSManagerDX9::redirectDrawPrimitiveUP(D3DPRIMITIVETYPE PrimitiveType, UINT PrimitiveCount, CONST void* pVertexStreamZeroData, UINT VertexStreamZeroStride) {
	return plugin->redirectDrawPrimitiveUP(PrimitiveType, PrimitiveCount, pVertexStreamZeroData, VertexStreamZeroStride);
}
HRESULT RSManagerDX9::redirectDrawIndexedPrimitive(D3DPRIMITIVETYPE Type, INT BaseVertexIndex, UINT MinVertexIndex, UINT NumVertices, UINT startIndex, UINT primCount) {
	return plugin->redirectDrawIndexedPrimitive(Type, BaseVertexIndex, MinVertexIndex, NumVertices, startIndex, primCount);
}
HRESULT RSManagerDX9::redirectDrawIndexedPrimitiveUP(D3DPRIMITIVETYPE PrimitiveType, UINT MinIndex, UINT NumVertices, UINT PrimitiveCount, CONST void * pIndexData, D3DFORMAT IndexDataFormat, CONST void * pVertexStreamZeroData, UINT VertexStreamZeroStride) {
...
HRESULT RSManagerDX9::redirectSetVertexShaderConstantF(UINT StartRegister, CONST float* pConstantData, UINT Vector4fCount) {
	return plugin->redirectSetVertexShaderConstantF(StartRegister, pConstantData, Vector4fCount);
}
HRESULT RSManagerDX9::redirectSetPixelShaderConstantF(UINT StartRegister, CONST float* pConstantData, UINT Vector4fCount) {
...
HRESULT RSManagerDX9::redirectCreateDepthStencilSurface(UINT Width, UINT Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample, DWORD MultisampleQuality, BOOL Discard, IDirect3DSurface9** ppSurface, HANDLE* pSharedHandle) {
